...
class DeepSeekProvider(AiProvider):
    ...
    def _handle_api_error(self, e: APIStatusError) -> None:
        if e.status_code == 401:
            raise AuthenticationError("DeepSeek API Key 无效或已过期") from e
        if e.status_code == 429:
            raise RateLimitError("DeepSeek API 请求频率超限") from e
        if e.status_code >= 500:
            raise ServerError(f"DeepSeek 服务器错误: {e.status_code}") from e
        raise
